...
public class JdbcXaSinkDerbyTest extends JdbcXaSinkTestBase {
    ...
    public void testTxEndedOnClose() throws Exception {
        sinkHelper.emit(
                JdbcTestFixture.TEST_DATA[0]); // don't snapshotState to prevent transaction from being prepared
        sinkHelper.close();
    ...
    public void testFailurePropagation() throws Exception {
        ...
        sinkHelper.emit(JdbcTestFixture.TEST_DATA[0]);
        sinkHelper.emit(JdbcTestFixture.TEST_DATA[0]); // duplicate
        try {
            sinkHelper.snapshotState(0);
        } catch (Exception e) {
