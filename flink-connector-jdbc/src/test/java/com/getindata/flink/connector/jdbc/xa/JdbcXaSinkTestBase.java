...
import com.getindata.flink.connector.jdbc.JdbcITCase;
...
import com.getindata.flink.connector.jdbc.JdbcExactlyOnceOptions;
...
public abstract class JdbcXaSinkTestBase extends JdbcTestBase {
    ...
    JdbcXaSinkFunction<JdbcTestFixture.TestEntry> buildAndInit() throws Exception {
        return buildAndInit(Integer.MAX_VALUE, getXaFacade());
    }

    JdbcXaSinkFunction<JdbcTestFixture.TestEntry> buildAndInit(int batchInterval, XaFacade xaFacade)
            throws Exception {
        return buildAndInit(batchInterval, xaFacade, createStateHandler());
    }

    static JdbcXaSinkFunction<JdbcTestFixture.TestEntry> buildAndInit(
            int batchInterval, XaFacade xaFacade, XaSinkStateHandler state) throws Exception {
        JdbcXaSinkFunction<JdbcTestFixture.TestEntry> sink =
    ...
    static JdbcXaSinkFunction<JdbcTestFixture.TestEntry> buildSink(
            XidGenerator xidGenerator,
            XaFacade xaFacade,
            XaSinkStateHandler state,
            int batchInterval) {
        JdbcOutputFormat<JdbcTestFixture.TestEntry, JdbcTestFixture.TestEntry, JdbcBatchStatementExecutor<JdbcTestFixture.TestEntry>> format =
                new JdbcOutputFormat<>(
                        ...
                        ctx ->
                                JdbcBatchStatementExecutor.simple(
                                        String.format(JdbcTestFixture.INSERT_TEMPLATE, JdbcTestFixture.INPUT_TABLE),
